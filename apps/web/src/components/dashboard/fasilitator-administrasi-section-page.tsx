...
type FacilitatorAdministrasiSectionContent = {
  title: string;
  description: string;
  items: string[];
};

export function FacilitatorAdministrasiSectionPage({
  eyebrow,
  content,
}: {
  eyebrow: string;
  content: FacilitatorAdministrasiSectionContent;
}) {
  return (
      ...
      <section className="panel rounded-[32px] p-8">
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <div className="rounded-[28px] border border-primary/10 bg-primary/[0.06] p-6">
            <p className="text-sm font-semibold text-primary">Fasilitator Administrasi</p>
            <p className="mt-4 text-sm leading-7 text-slate-600">
              Halaman ini disiapkan untuk mengawal kelengkapan administrasi, verifikasi berkas, dan pendataan resmi seluruh sekolah dampingan.
            </p>
          </div>
        </div>
      </section>
            ...
            <CardContent className="flex items-start justify-between gap-4 p-6">
              <div>
                ...
                <p className="mt-2 text-sm leading-7 text-slate-500">
                  Struktur halaman sudah siap dan bisa dilanjutkan ke tabel, form, serta alur kerja administrasi sekolah.
                </p>
              </div>
