...
import { GoogleDocsRpdPksEditor } from "@/components/school/google-docs-rpd-pks-editor";
...
function PageHeaderSummary({
  ...
  const { rpkpGrand, rabGrand } = precomputedTotals ?? useMemo(() => {
    let rpkp = 0;
    let rab = 0;
...
// ============== MAIN PAGE COMPONENT ==============
export function FacilitatorAdministrasiPraBimtekDetailPage({ npsn }: { npsn: string }) {
  const session = useAuthStore((s) => s.session);
  ...
  const [mainTab, setMainTab] = useState<MainTabKey>("administrasi");
  ...
  async function loadDetail(force = false) {
    ...
    try {
      ...
      if (!cancelled) setLoadError(err instanceof Error ? err.message : "Gagal memuat detail Pra-Bimtek sekolah.");
    } finally {
      if (!cancelled) {
        setLoading(false);
        setRefreshing(false);
  ...
  return (
    <div className="space-y-6">
      {/* Print Portal Root (hanya untuk clone saat window.print) */}
      <div id="print-portal-root" aria-hidden className="print-only" />

      {/* Breadcrumb Back */}
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="ghost" size="sm" className="gap-1 text-slate-600 hover:text-slate-900" onClick={() => history.length > 1 ? history.back() : (window.location.href = "/fasilitator-administrasi/pra-bimtek")}>
          <ChevronLeft className="h-4 w-4" />
          Kembali ke Daftar Pra-Bimtek
        </Button>
        <Badge className="border-slate-200 bg-slate-50 text-slate-700" variant="outline">
          <FileCheck2 className="-ml-0.5 mr-1 h-3 w-3" />
          Module Pra-Bimtek · Fasilitator Administrasi
        </Badge>
        <Badge className="border-emerald-300 bg-emerald-50 text-emerald-800" variant="outline">
          <ClipboardCheck className="-ml-0.5 mr-1 h-3 w-3" />
          ★ Checklist Dokumen Administrasi · Tab Utama
        </Badge>
        {refreshing ? (
          <Badge className="border-sky-200 bg-sky-50 text-sky-800 animate-pulse" variant="outline">
            <RefreshCw className="-ml-0.5 mr-1 h-3 w-3 animate-spin" />
            Refresh data…
          </Badge>
        ) : null}
        {loadError ? (
          <Badge className="border-rose-200 bg-rose-50 text-rose-800" variant="outline">{loadError}</Badge>
        ) : null}
      </div>
      ...
      <PageHeaderSummary
        ...
        precomputedTotals={{ rpkpGrand, rabGrand }}
      />

      {/* Identity Header */}
      <SchoolIdentityHeader profile={displayProfile} />
        ...
        <DetailTabLazyWrap active={mainTab === "rpd"} skeletonRows={16}>
          {mainTab === "rpd" ? (
            <div className="space-y-4">
              ...
              />
              <details className="text-xs text-slate-400 border border-slate-200 rounded-lg px-3 py-2" open={false}>
                <summary className="cursor-pointer hover:text-slate-600">
                  ▸ View Google Docs RPD (fallback integrasi Google — tetap tersedia)
                </summary>
                <GoogleDocsRpdPksEditor
                  npsn={String(displayProfile.npsn || npsn)}
                  schoolName={displayProfile.schoolName}
                  initialKind="RPD"
                />
                <div className="mt-2">
                  <PlaceholderDokumenFiks
                    kode="⑤"
                    nama="Rencana Penggunaan Dana (RPD)"
                    icon={<FileBarChart className="h-5 w-5" />}
                    profile={displayProfile}
                    description="Legacy static preview — sekarang pakai HTML rich text editor di atas (self-hosted, tanpa Google Docs)."
                    printId="print-rpd"
                    onPreview={(id) => setPrintPreviewId(id)}
                  />
                </div>
        ...
        <DetailTabLazyWrap active={mainTab === "pks"} skeletonRows={16}>
          {mainTab === "pks" ? (
            <div className="space-y-4">
              ...
              <details className="text-xs text-slate-400 border border-slate-200 rounded-lg px-3 py-2" open={false}>
                <summary className="cursor-pointer hover:text-slate-600">
                  ▸ View Google Docs PKS (fallback integrasi Google — tetap tersedia)
                </summary>
                <GoogleDocsRpdPksEditor
                  npsn={String(displayProfile.npsn || npsn)}
                  schoolName={displayProfile.schoolName}
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    window.print();
                    window.setTimeout(() => clearPrintPortal(), 300);
                  }}
      ...
      {/* GLOBAL PRINT STYLESHEET — A4 ukuran pas, hapus UI selain konten cetak */}
      <style jsx global>{`
        @media print {
          @page {
            size: A4;
          ...
          #print-portal-root .print-a4-landscape {
            page: landscape;
          }
          ...
          #print-portal-root thead {
            display: table-header-group;
          }
          #print-portal-root tfoot {
            display: table-footer-group;
          }
          #print-portal-root .rounded-\[24px\],
          #print-portal-root .rounded-\[28px\],
          #print-portal-root .rounded-\[20px\] {
            border-radius: 0 !important;
            box-shadow: none !important;
            border: 1px solid #000 !important;
          }
          #print-portal-root .bg-slate-50 {
            background-color: #f1f5f9 !important;
          }
          #print-portal-root .bg-primary\/\[0\.05\],
          #print-portal-root .bg-primary\/\[0\.06\] {
            background-color: #eef2ff !important;
          }
          #print-portal-root .bg-emerald-50\/60 {
            background-color: #ecfdf5 !important;
          }
          #print-portal-root .bg-slate-900,
          #print-portal-root .bg-black {
            background-color: #0f172a !important;
            color: white !important;
          }
          #print-portal-root .text-slate-400,
          #print-portal-root .text-slate-500 {
            color: #334155 !important;
          }
          #print-portal-root .border-dashed {
